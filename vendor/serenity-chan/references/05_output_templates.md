...
## 12. 交付验证凭证
- proof_status: PASS
- artifacts: core structured artifact hash
- validators: command, exit code, status
```
...
行动候选首位：____。
正式决策对象：____。
决策模式：clear_decision_candidate / research_lead / candidate_cluster / comparison_not_decision_grade / single_research_object。
...
## 3.1 AI 深研 Dossier
| 公司 | Dossier 状态 | 已读源 | 观察 | 推断 | 判断 | Claim | 因果步骤 | 行动条件 |
|---|---|---:|---:|---:|---:|---:|---:|---|

正式候选对比必须为每个候选生成 `ai_research_dossier` 加 `ai_research_overlay` 或 `ai_review_outcome`，并通过 `validate_and_merge_ai_overlay.py` 合并。`NOT_RUN` 只用于 AI 合并前的 deterministic baseline 或诊断输出。
...
若任何行是 `MISMATCH`，`ranking_validity` 必须为 `INVALID`，不能输出正式决策对象。
若任何行是 `PARTIAL` 或 `DATA_GATED`，或存在 high/critical `research_debt`，`ranking_validity` 必须为 `PARTIAL`，`final_decision.decision_mode` 不能设为 `clear_decision_candidate`。
`candidate_pool_semantic_coherence` 仅在 `SAME_LAYER` 时允许输出正式决策对象；其他状态只能输出研究优先级或诊断集合。
