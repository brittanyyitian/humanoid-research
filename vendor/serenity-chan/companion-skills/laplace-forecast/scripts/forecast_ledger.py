...
def normalize_claim(claim: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(claim.get("resolution_probe"), dict):
        normalized["resolution_probe"] = claim["resolution_probe"]
    if claim.get("source_symbol"):
        normalized["source_symbol"] = str(claim["source_symbol"])
    return normalized
