...
def _context_lines(strategy_input: Mapping[str, Any]) -> list[str]:
    lines: list[str] = []
    market_wrapper: Mapping[str, Any] = _as_mapping(strategy_input.get("market_context"))
    market_regime: Mapping[str, Any] = _as_mapping(market_wrapper.get("market_regime"))
    if market_wrapper.get("market_regime_present") is True and market_regime:
        summary: Mapping[str, Any] = _as_mapping(market_regime.get("summary"))
        lines.extend(
            [
                "",
                "## 市场环境",
                f"- 状态：{summary.get('regime_status')}",
                f"- 基准数量：{summary.get('benchmark_count')}",
                f"- 边界：{market_regime.get('decision_boundary')}",
            ]
        )
    portfolio_wrapper: Mapping[str, Any] = _as_mapping(strategy_input.get("portfolio_context"))
    portfolio_context: Mapping[str, Any] = _as_mapping(portfolio_wrapper.get("portfolio_context"))
    if portfolio_wrapper.get("portfolio_context_present") is True and portfolio_context:
        constraints: Mapping[str, Any] = _as_mapping(portfolio_context.get("constraints"))
        lines.extend(
            [
                "",
                "## 组合上下文",
                f"- 账户范围：{portfolio_context.get('account_scope')}",
                f"- 持仓数量：{len(_as_list(portfolio_context.get('positions')))}",
                f"- 风险偏好：{constraints.get('risk_profile') or ''}",
                f"- 边界：{portfolio_context.get('decision_boundary')}",
            ]
        )
    return lines
...
def render_strategy_report(judgment: Mapping[str, Any], *, strategy_input_path: Optional[Path] = None) -> str:
    ...
    strategy_input: Mapping[str, Any] = _load_json(strategy_input_path) if strategy_input_path else {}
    lines: list[str] = [
        "# 策略预测与行动计划",
        "",
        "## Forecast",
    ...
    if ledger_claims:
        lines.extend(["", "## Ledger Claims", "| Claim | Probability | Resolution Date | Criteria | Probe |", "|---|---:|---|---|---|"])
        for row in ledger_claims:
            if isinstance(row, Mapping):
                probe: Mapping[str, Any] = _as_mapping(row.get("resolution_probe"))
                probe_label: str = str(probe.get("type") or "")
                lines.append(f"| {row.get('claim')} | {row.get('probability')} | {row.get('resolution_date')} | {row.get('resolution_criteria')} | {probe_label} |")
    return "\n".join(lines).rstrip() + "\n"
