...
THEME_REGISTRY_PATH: Path = Path(__file__).resolve().parents[1] / "assets" / "theme_packs.json"
        ...
        "layers": [
            {
                ...
                "evidence_to_seek": ["减速器/丝杠/传动收入占比", "客户验证或批量订单", "产能与良率", "价格和毛利率趋势"],
                "candidates": [
                    ("688017.SH", "绿的谐波", "谐波减速器核心供应商"),
                    ("002896.SZ", "中大力德", "减速器和传动部件"),
                    ("603728.SH", "鸣志电器", "运动控制和精密传动相关部件"),
                    ("002472.SZ", "双环传动", "精密齿轮和传动部件"),
                    ("300580.SZ", "贝斯特", "精密零部件和丝杠方向线索"),
                    ("301596.SZ", "瑞迪智驱", "精密传动和制动部件线索"),
                ],
            },
            {
                ...
                "candidates": [
                    ...
                    ("688320.SH", "禾川科技", "伺服系统和运动控制"),
                    ("688160.SH", "步科股份", "工业自动化和控制部件"),
                    ("002979.SZ", "雷赛智能", "运动控制和驱动系统"),
                    ("002527.SZ", "新时达", "伺服驱动、控制和机器人本体"),
                ],
            },
            {
                "layer": "视觉、力控与传感感知",
                "bottleneck_question": "谁能把视觉、力觉、触觉和检测能力变成机器人可量产感知方案？",
                "evidence_to_seek": ["机器人感知产品收入", "客户验证", "传感器/视觉订单", "算法和硬件协同"],
                "candidates": [
                    ("688322.SH", "奥比中光", "3D 视觉感知"),
                    ("603662.SH", "柯力传感", "力传感和称重传感线索"),
                    ("688686.SH", "奥普特", "机器视觉部件和系统"),
                    ("688207.SH", "格灵深瞳", "视觉算法和应用线索"),
                ],
            },
            {
                "layer": "本体、系统集成与应用落地",
                ...
                "candidates": [
                    ...
                    ("002031.SZ", "巨轮智能", "机器人和智能装备线索"),
                    ("603666.SH", "亿嘉和", "特种机器人和电力巡检应用"),
                    ("300607.SZ", "拓斯达", "工业机器人和自动化应用"),
                ],
            },
        ],
        "downgraded": [
...
def _load_theme_registry() -> Mapping[str, Any]:
    try:
        payload: Any = json.loads(THEME_REGISTRY_PATH.read_text(encoding="utf-8"))
    except Exception:
        return {}
    return payload if isinstance(payload, Mapping) else {}


def _registry_record(theme_key: str) -> Mapping[str, Any]:
    registry: Mapping[str, Any] = _load_theme_registry()
    for item in registry.get("packs", []) if isinstance(registry.get("packs"), list) else []:
        if isinstance(item, Mapping) and str(item.get("key") or "") == theme_key:
            return item
    return {}


def _seed_status(record: Mapping[str, Any]) -> str:
    as_of_text: str = str(record.get("as_of") or "")
    try:
        as_of_date = datetime.strptime(as_of_text, "%Y-%m-%d").date()
        freshness_days = int(record.get("freshness_days") or 180)
    except Exception:
        return "UNKNOWN"
    age_days: int = (datetime.now(timezone.utc).date() - as_of_date).days
    return "FRESH" if age_days <= freshness_days else "STALE"
...
def build_universe(theme: str) -> dict[str, Any]:
    ...
    registry_record: Mapping[str, Any] = _registry_record(key)
    ...
    deep_scan_task: str = (
        "候选池已达到 20 个以上时，继续补充遗漏层级和边缘候选；不足 20 个时，按价值链层级扩展到至少 20 个候选，并标注主层级、收入传导和证据来源。"
        if len(candidates) >= 20
        else "按价值链层级扩展到至少 20 个候选，并标注每个候选的主层级、收入传导和证据来源。"
    )
    return {
        ...
        "seed_registry": {
            "theme_key": key,
            "registry_path": str(THEME_REGISTRY_PATH),
            "as_of": str(registry_record.get("as_of") or ""),
            "freshness_days": registry_record.get("freshness_days"),
            "freshness_status": _seed_status(registry_record),
            "source_notes": str(registry_record.get("source_notes") or ""),
        },
