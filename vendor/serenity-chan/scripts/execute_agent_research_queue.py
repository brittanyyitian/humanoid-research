...
def _candidate_result_status(work_item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        "dossier_valid": False,
        "dossier_quality_score": None,
        "dossier_quality_band": "",
        "dossier_quality_delivery_allowed": False,
        "result_valid": False,
        "result_type": "",
    ...
    try:
        ...
        quality: dict[str, Any] = score_dossier(dossier, evidence_context=evidence_context)
        dossier_status: str = str(dossier.get("research_status") or "")
        row["dossier_valid"] = True
        row["dossier_quality_score"] = quality.get("score")
        row["dossier_quality_band"] = quality.get("quality_band")
        row["dossier_quality_delivery_allowed"] = bool(quality.get("delivery_allowed"))
        row["research_status"] = dossier_status
        if quality.get("delivery_allowed") is not True:
            row["errors"].append(
                "dossier quality blocked: "
                + "; ".join(str(item) for item in quality.get("blocking_issues", []) if item)
            )
            row["next_action"] = "Deepen ai_research_dossier.json until score_ai_research_dossier.py passes."
            return row
