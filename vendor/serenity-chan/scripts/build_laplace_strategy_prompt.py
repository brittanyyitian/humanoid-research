...
def build_strategy_prompt(strategy_input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "instruction": (
            ...
            "candidate-pool coherence, or strategy_readiness.allowed_strategy_modes. Convert research debt into next evidence, "
            "triggers, invalidation, and action discipline. Use market_context to reason about broad-market background and "
            "relative strength. Use portfolio_context only when it is present before producing portfolio-level add, reduce, "
            "rebalance, or position-size actions."
        ),
        "required_reads": [
        ...
        "hard_constraints": [
            ...
            "Use strategy_readiness.status and allowed_strategy_modes as the action boundary.",
            "Ledger claims must be observable and resolvable; include resolution_probe when a claim can be machine-checked.",
            "Portfolio-level actions require portfolio_context.portfolio_context_present=true.",
        ],
