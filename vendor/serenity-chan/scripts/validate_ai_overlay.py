...
DEFAULT_PRIMARY_CONFIDENCE_FLOOR: float = 0.65


def primary_confidence_floor(calibration_policy: Optional[Mapping[str, Any]] = None) -> float:
    """L0/L1 confidence floor, raised by the calibration policy when history is poor.

    Policy source order: explicit mapping, then the SERENITY_CALIBRATION_POLICY
    env path, then the default. An unreadable policy never lowers the bar.
    """
    policy: Optional[Mapping[str, Any]] = calibration_policy
    if policy is None:
    ...
    if isinstance(policy, Mapping):
        try:
            floor: float = float(policy.get("h4_h5_confidence_floor", DEFAULT_PRIMARY_CONFIDENCE_FLOOR))
            return max(DEFAULT_PRIMARY_CONFIDENCE_FLOOR, min(floor, 0.95))
        except (TypeError, ValueError):
            return DEFAULT_PRIMARY_CONFIDENCE_FLOOR
    return DEFAULT_PRIMARY_CONFIDENCE_FLOOR
...
def validate_overlay(
    ...
    *,
    calibration_policy: Optional[Mapping[str, Any]] = None,
) -> dict[str, Any]:
    errors: list[str] = []
    warnings: list[str] = []
    confidence_floor: float = primary_confidence_floor(calibration_policy)
    if confidence_floor > DEFAULT_PRIMARY_CONFIDENCE_FLOOR:
        warnings.append(
            f"calibration policy raised the L0/L1 confidence floor to {confidence_floor:.2f}"
        )
    ...
    for idx, item in enumerate(evidence_refs):
        ...
        if confidence is None or confidence < 0 or confidence > 1:
            ...
            confidence = 0.0
        if level in {"L0", "L1"} and confidence >= confidence_floor:
    ...
    if (serenity_fit >= 0.72 or ai_confidence == "HIGH") and strong_primary_refs == 0:
        errors.append(
            f"high-fit or high-confidence overlay requires at least one L0/L1 evidence reference with confidence >= {confidence_floor:.2f}"
        )
    ...
    if str(supported) in {"H4", "H5"}:
        ...
        if strong_primary_refs == 0:
            errors.append(
                f"H4/H5 evidence_supported_growth requires at least one L0/L1 evidence reference with confidence >= {confidence_floor:.2f}"
            )
