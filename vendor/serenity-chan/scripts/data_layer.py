...
'''
    ...
    def _extract_label_value(
        ...
        *,
        ...
        for page in pages:
            text: Any = str(page.get("text") or "")
            lines: Any = text.splitlines()
            for index, line in enumerate(lines):
                clean: Any = cls._normalize_pdf_line(line)
                candidates: Any = [clean]
                if not cls._pdf_number_tokens(clean) and index + 1 < len(lines):
                    candidates.append(
                        f"{clean} {cls._normalize_pdf_line(lines[index + 1])}".strip()
                    )
                for candidate in candidates:
                    clean_lower: Any = candidate.lower()
...
class CninfoFinancialReportsProvider(CninfoFinancialReportBase, PdfTextExtractionMixin):
    ...
    def _extract_cninfo_report_period(self, report: Mapping[str, Any], *, raw_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        assets_value: Any = fields.get("assets")
        liabilities_value: Any = fields.get("liabilities")
        equity_value: Any = fields.get("equity")
        if (
            isinstance(assets_value, (int, float))
            and isinstance(liabilities_value, (int, float))
            and assets_value > 0
            and liabilities_value >= 0
        ):
            derived_equity: Any = assets_value - liabilities_value
            balance_gap_ratio: Any = (
                abs(derived_equity - equity_value) / max(abs(assets_value), 1.0)
                if isinstance(equity_value, (int, float))
                else 1.0
            )
            if derived_equity >= 0 and balance_gap_ratio > 0.01:
                replaced_evidence: Any = evidence.get("equity")
                fields["equity"] = derived_equity
                evidence["equity"] = {
                    "page_number": (evidence.get("assets") or {}).get("page_number"),
                    "line": (
                        "Derived from the official consolidated balance-sheet identity: "
                        f"assets {assets_value} - liabilities {liabilities_value} = equity {derived_equity}"
                    ),
                    "value_index": 0,
                    "derivation": "assets_minus_liabilities",
                    "source_evidence": {
                        "assets": evidence.get("assets"),
                        "liabilities": evidence.get("liabilities"),
                        "replaced_equity_extraction": replaced_evidence,
                    },
                }
    ...
    def _requires_financial_sector_profile(
        ...
        # A disclosure title can mention “证券交易所” even when the issuer is
        # industrial. Use the issuer name so exchange wording cannot trigger the
        # financial-company statement rules.
        compact: Any = cls._compact_cn(str(evidence.get("name") or ""))
...
'''
    ...
    def _extract_hkex_report_period(self, report: Mapping[str, Any], *, raw_dir: Optional[Path] = None) -> Dict[str, Any]:
        ...
        # HK annual reports commonly put audited statements after page 160.
        # Read the full filing before deciding that financial evidence is incomplete.
        page_bundle: Any = self._extract_pdf_pages(pdf_path, max_pages=360, timeout=45)
        if not page_bundle.get("ok"):
            return {"status": "FAILED", "errors": page_bundle.get("errors", ["PDF text extraction failed"])}
        ...
        income_pages = [
            page for page in income_pages
            if "statement of comprehensive income" not in str(page.get("text") or "").lower()
        ]
        position_pages: Any = self._page_texts_containing(pages, "balance sheet")
        if not position_pages:
            position_pages = self._page_texts_containing(pages, "statement of financial position")
        cashflow_pages: Any = self._page_texts_containing(pages, "cash flow statement")
        if not cashflow_pages:
        ...
        income_columns: Any = 4 if "company company" in income_text.lower() else 2
        income_index: Any = 0
        position_columns: Any = 4 if "company company" in position_text.lower() else 2
        cashflow_columns: Any = 4 if "company company" in cashflow_text.lower() else 2
        ...
        if value is None:
            value, source = self._extract_label_value(
                income_pages,
                ["revenue"],
                expected_columns=income_columns,
                value_index=income_index,
            )
        put("revenue", value, source)
        for field, labels in {
            ...
            "total_net_profit": ["profit for the year", "profit for the period", "net loss", "net profit"],
        ...
        for field, labels in {
            "assets": ["total assets"],
            "equity": ["total equity", "total shareholders’ equity", "total shareholders' equity"],
            ...
            value, source = self._extract_label_value(
                position_pages,
                labels,
                expected_columns=position_columns,
                value_index=0,
            )
            put(field, value, source)
        ...
        for field, labels in {
            ...
            value, source = self._extract_label_value(
                cashflow_pages,
                labels,
                expected_columns=cashflow_columns,
                value_index=0,
            )
            put(field, value, source)
    ...
    def fetch(self, symbol: SymbolInfo, dataset: Dataset, **kwargs: Any) -> DataResult:
        ...
        try:
            ...
            for report in reports:
                if report.get("download_status") != "OK" or not report.get("pdf_path"):
                    continue
                report_title: Any = str(report.get("title") or "")
                if (
                    report_title.lstrip().startswith("(")
                    and ("notice of agm" in report_title.lower() or "general mandate" in report_title.lower())
                ):
                    report["line_extraction"] = {
                        "status": "NOT_APPLICABLE",
                        "reason": "AGM/circular bundle is not the standalone periodic financial report.",
                    }
                    continue
