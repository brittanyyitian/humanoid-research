...
def _validate_seed_registry(payload: Mapping[str, Any], errors: list[str]) -> None:
    registry: Mapping[str, Any] = _as_mapping(payload.get("seed_registry"), "seed_registry", errors)
    for key in ["theme_key", "registry_path", "as_of", "freshness_days", "freshness_status", "source_notes"]:
        if key not in registry:
            errors.append(f"seed_registry missing {key}")
    for key in ["theme_key", "registry_path", "as_of", "freshness_status", "source_notes"]:
        if not str(registry.get(key) or "").strip():
            errors.append(f"seed_registry.{key} must not be empty")
    freshness_status: str = str(registry.get("freshness_status") or "")
    if freshness_status not in {"FRESH", "STALE", "UNKNOWN"}:
        errors.append(f"seed_registry.freshness_status is invalid: {freshness_status}")
    freshness_days: Any = registry.get("freshness_days")
    if freshness_days is not None and (not isinstance(freshness_days, int) or freshness_days < 1):
        errors.append("seed_registry.freshness_days must be null or a positive integer")
...
def validate_universe(payload: Mapping[str, Any]) -> list[str]:
    ...
    _validate_seed_registry(payload, errors)
