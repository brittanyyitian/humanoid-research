...
def _load_optional_object(path: Optional[Path], label: str) -> Mapping[str, Any]:
    if path is None:
        return {}
    payload: Mapping[str, Any] = _load_json(path)
    if not payload:
...
def _load_portfolio_context(path: Optional[Path]) -> Mapping[str, Any]:
    payload: Mapping[str, Any] = _load_optional_object(path, "portfolio_context")
    if not payload:
        return {}
    errors: list[str] = validate_portfolio_context(payload)
    if errors:
        raise ValueError(f"portfolio_context is invalid: {'; '.join(errors)}")
    return payload
...
def _forecast_variables(report: Mapping[str, Any]) -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            "variable": "市场环境与相对强弱",
            "role": "regime",
            "direction": "risk-on broadens action room; risk-off tightens triggers",
            "observability": "medium",
            "change_speed": "fast",
            "dominant_lens": "market behavior",
            "confidence": "medium",
            "why": "同样的个股结构在不同市场环境下对应不同触发器、容错率和机会成本。",
        },
    ]
...
def build_strategy_input(
    ...
    *,
    ...
    market_regime_path: Optional[Path] = None,
    ...
    market_regime: Mapping[str, Any] = _load_optional_object(market_regime_path, "market_regime")
    portfolio_context: Mapping[str, Any] = _load_portfolio_context(portfolio_context_path)
    ...
    strategy_readiness: dict[str, Any] = evaluate_strategy_readiness(report)
    if strategy_readiness.get("formal_report_ready") is not True:
        raise ValueError("strategy_readiness.formal_report_ready must be true before strategy input handoff")
    ...
    return {
        ...
        "market_context": {
            "market_regime_path": str(market_regime_path.resolve()) if market_regime_path else "",
            "market_regime_present": bool(market_regime),
            "market_regime": dict(market_regime),
            "policy": "Use market regime as background evidence for trigger strictness, opportunity cost, and relative-strength interpretation.",
        },
        "portfolio_context": {
            "portfolio_context_path": str(portfolio_context_path.resolve()) if portfolio_context_path else "",
            "portfolio_context_present": bool(portfolio_context),
            "portfolio_context": dict(portfolio_context),
            "policy": "Portfolio-level buy/add/reduce/allocation actions require explicit portfolio context; otherwise output watchlist, trigger, and evidence-first decisions.",
        },
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--market-regime", help="optional market_regime.json used for broad market and benchmark context")
    parser.add_argument("--portfolio-context", help="optional portfolio_context.json used before portfolio-level actions")
    ...
    try:
        ...
        payload: dict[str, Any] = build_strategy_input(
            ...
            portfolio_context_path=Path(args.portfolio_context) if args.portfolio_context else None,
        )
