...
def validate_strategy_judgment(
    ...
    *,
    ...
    readiness: Mapping[str, Any] = strategy_input.get("strategy_readiness") if isinstance(strategy_input.get("strategy_readiness"), Mapping) else {}
    allowed_modes: set[str] = {str(item) for item in readiness.get("allowed_strategy_modes", []) if str(item)}
    if strategy_input_path is not None and "allocation" not in allowed_modes:
        decision_text = _string(payload.get("decision")) + " " + " ".join(str(item) for item in payload.get("action_plan", []) if item)
        action_tokens: set[str] = {"买入", "加仓", "建仓", "配置", "仓位", "allocation", "position", "buy", "add"}
        gate_tokens: set[str] = {"观察", "等待", "触发", "条件", "补证", "跟踪", "watch", "wait", "trigger", "evidence"}
        if _contains_any(decision_text, action_tokens) and not _contains_any(decision_text, gate_tokens):
            errors.append("strategy_readiness blocks allocation, so action_plan must stay watchlist/evidence-trigger based")
    portfolio_wrapper: Mapping[str, Any] = strategy_input.get("portfolio_context") if isinstance(strategy_input.get("portfolio_context"), Mapping) else {}
    portfolio_present: bool = portfolio_wrapper.get("portfolio_context_present") is True
    if strategy_input_path is not None and not portfolio_present:
        decision_text = _string(payload.get("decision")) + " " + " ".join(str(item) for item in payload.get("action_plan", []) if item)
        portfolio_action_tokens: set[str] = {"加仓", "减仓", "仓位", "调仓", "配置比例", "组合权重", "position size", "rebalance", "add position", "trim"}
        if _contains_any(decision_text, portfolio_action_tokens):
            errors.append("portfolio-level actions require portfolio_context with positions and constraints")
    return errors
