...
def _artifact_ref(value: Any) -> str:
    text: str = _text(value, "NA")
    if text == "NA":
        return text
    path: Path = Path(text)
    parts: tuple[str, ...] = path.parts
    if "data" in parts:
        index: int = parts.index("data")
...
def _research_debt_lines(rows: Sequence[Mapping[str, Any]]) -> list[str]:
    ...
    for row in rows:
        ...
        impact: str = display_label(row.get("decision_impact"), "影响待核验")
...
def _render_full_research(report: Mapping[str, Any], *, comparison_markdown: str) -> str:
    ...
    ai_dossier_rows: dict[str, Mapping[str, Any]] = _first_by_symbol(_mapping_rows(report.get("ai_research_dossier_matrix")))
    ...
    for candidate in candidates:
        ...
        ai_dossier: Mapping[str, Any] = ai_dossier_rows.get(symbol, {})
        ...
        lines.extend([
            ...
            f"- AI 深研质量：`{_text(ai_dossier.get('quality_score'), 'NA')}` / `{display_label(ai_dossier.get('quality_band'), 'NA')}`；质量可交付 `{display_bool(ai_dossier.get('quality_delivery_allowed'))}`",
            f"- 数据包：`{_artifact_ref(candidate.get('data_package_path'))}`",
            "",
            "### Serenity 研究假设",
