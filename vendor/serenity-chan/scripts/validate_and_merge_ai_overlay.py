...
def load_validated_dossiers(
    ...
    for value in values:
        ...
        quality: dict[str, Any] = score_dossier(dossier, evidence_context=context)
        if quality.get("delivery_allowed") is not True:
            issues: str = "; ".join(str(item) for item in quality.get("blocking_issues", []) if item)
            raise ValueError(f"AI research dossier quality blocked for {symbol}: score={quality.get('score')}; {issues}")
