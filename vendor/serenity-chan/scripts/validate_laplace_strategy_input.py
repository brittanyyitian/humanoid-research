...
try:
    from validate_strategy_readiness import validate_strategy_readiness
    ...
    from scripts.validate_strategy_readiness import validate_strategy_readiness
...
def validate_strategy_input(payload: Mapping[str, Any]) -> list[str]:
    ...
    strategy_readiness: Mapping[str, Any] = _as_mapping(payload.get("strategy_readiness"), "strategy_readiness", errors)
    errors.extend(f"strategy_readiness: {error}" for error in validate_strategy_readiness(strategy_readiness))
    if strategy_readiness.get("formal_report_ready") is not True:
        errors.append("strategy_readiness.formal_report_ready must be true for Laplace strategy input")
    ...
    market_context: Any = payload.get("market_context")
    if market_context is not None:
        context_payload: Mapping[str, Any] = _as_mapping(market_context, "market_context", errors)
        if not isinstance(context_payload.get("market_regime_present"), bool):
            errors.append("market_context.market_regime_present must be a boolean")
        market_regime: Any = context_payload.get("market_regime")
        if context_payload.get("market_regime_present") is True:
            market_regime_payload: Mapping[str, Any] = _as_mapping(market_regime, "market_context.market_regime", errors)
            summary_payload: Mapping[str, Any] = _as_mapping(market_regime_payload.get("summary"), "market_context.market_regime.summary", errors)
            if summary_payload.get("regime_status") not in {"RISK_ON", "MIXED", "RISK_OFF", "DATA_GATED"}:
                errors.append("market_context.market_regime.summary.regime_status is unknown")

    portfolio_context_wrapper: Any = payload.get("portfolio_context")
    if portfolio_context_wrapper is not None:
        wrapper_payload: Mapping[str, Any] = _as_mapping(portfolio_context_wrapper, "portfolio_context", errors)
        if not isinstance(wrapper_payload.get("portfolio_context_present"), bool):
            errors.append("portfolio_context.portfolio_context_present must be a boolean")
        context_payload = wrapper_payload.get("portfolio_context")
        if wrapper_payload.get("portfolio_context_present") is True:
            if not isinstance(context_payload, Mapping):
                errors.append("portfolio_context.portfolio_context must be an object when portfolio_context_present is true")
            else:
                context_errors: list[str] = validate_portfolio_context(context_payload)
                errors.extend(f"portfolio_context.portfolio_context: {error}" for error in context_errors)
    return errors
