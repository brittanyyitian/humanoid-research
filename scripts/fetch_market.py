...
def fetch_tencent(entity: dict) -> dict:
    ...
    return {
        ...
        "changePct": float(parts[32]) if parts[32] else None,
        "turnoverAmount": yuan_to_cn(float(parts[37]) * 10_000 if parts[37] else None),
...
def fetch_quote(entity: dict) -> dict:
    try:
        return fetch_eastmoney(entity)
    except Exception as eastmoney_error:  # noqa: BLE001 - fallback keeps source diversity.
        try:
            row = fetch_tencent(entity)
            row["fallbackFrom"] = {
                "provider": "eastmoney",
                "error": str(eastmoney_error),
            }
            return row
        except Exception as tencent_error:  # noqa: BLE001 - caller records final error.
            raise RuntimeError(
                f"eastmoney failed: {eastmoney_error}; tencent failed: {tencent_error}"
            ) from tencent_error
